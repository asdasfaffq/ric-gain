...
Selected core code and results for **RIC-Gain** — a measured-gain refresh policy that
...
- `results/scifact_rank_tests.csv` — RIC-Gain vs nine refresh-ordering baselines on SciFact (MiniLM->BGE), paired Wilcoxon with Holm correction; corresponds to the paper's **main ranking results table**. RIC-Gain (`ric_mean = 0.733`) is rank-first and Holm-significant over every baseline (e.g. vs the learned scheduler, delta = +0.053, Holm p = 6.2e-7).
